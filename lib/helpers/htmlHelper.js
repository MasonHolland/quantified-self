...
class HTMLHelper {
  static newTableEntry(val, table) {
    return `
      <tr class='table-row'>
        <td class='table-name ${val['id']}'>${val['name']}</td>
        <td class='table-cal ${val['id']}'>${val['calories']}</td>
        <td class='delete-button'><input type='image' class='delete-food-${table} ${val['id']}' src='https://i.imgur.com/Ea2X1B0.png'/></td>
      </tr>`
  }
  ...
  static totalMealCalories(val) {
    return `
      <tr class='table-row meal-calories'>
        ...
        <td class='${val.name}-calories meal-cals-val'>${val.calories}</td>
      </tr>
