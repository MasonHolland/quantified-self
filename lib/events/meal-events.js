...
$('.meal-selection .btn').on('click', function addFoodstoMeal(event) {
  ...
  $('.meal-table-body').trigger(refreshCalories(mealName));
});
...
$(document).on('click','.delete-food-meal-table',function(){
  ...
  Meal.deleteFood(mealID, foodID).then(function(data){
    $('.meal-table-body').trigger(refreshCalories(mealID));
    console.log("hello")
  });
  event.preventDefault();
});
...
function refreshCalories(meal) {
  if (meal == '1' || meal == 'Breakfast') {
    calculateNewTotalCalories('Breakfast')
  } else if (meal == '2') {
    calculateNewTotalCalories('Lunch')
  } else if (meal == '3') {
    calculateNewTotalCalories('Dinner')
  } else if (meal == '4') {
    calculateNewTotalCalories('Snack')
  }
}
